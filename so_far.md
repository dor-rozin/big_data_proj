...
| `dashboard/` — Streamlit | Ohad + Dor | Runs end-to-end | 7 KPI charts, candles + crosshair, Woodies CCI/Stoch/MACD sub-panels, and **two on-demand LLM analysts** writing back to `stock_analysis`. Demo-ready 2026-09-03. Same day: sidebar "Refresh data" button (Docker-out-of-Docker re-run of the Spark job from the browser) and a "Live price ticker" reading `market.prices.v1` directly via a background Kafka consumer (`dashboard/kafka_tail.py`), bypassing Elasticsearch/Spark entirely — both verified live |
...
rather than changing it unreviewed. **Now stale, see below**: Dor's same-day
change moved the LLM call out of `spark/pipeline.py` entirely, so the default
mismatch this entry describes needs re-checking against the new code, not
